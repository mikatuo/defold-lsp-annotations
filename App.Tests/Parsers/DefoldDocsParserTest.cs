...
namespace App.Tests.Parsers
{
    ...
    public class DefoldDocsParserTest
    {
        ...
        {
            RawApiReference apiReference = MessageApiReference();

            var sut = new DefoldDocsParser(apiReference);
            DefoldApi result = sut.Parse();

            result.Namespace.Should().Be("msg");
            result.Name.Should().Be("Message");
            result.Brief.Should().Be("Messaging API documentation");
            result.Description.Should().Be("Functions for passing messages and constructing URL objects.");

            result.Functions.Should().HaveCount(2);

            result.Functions.Should().BeEquivalentTo(new[] {
                    new DefoldFunction {
                        Name = "msg.url",
                        Brief = "creates a new URL from separate arguments",
                        Description = new [] { "creates a new URL from separate arguments" },
                        ReturnValues = new [] {
        ...
        RawApiReference MessageApiReference()
        {
            return new RawApiReference {
                Info = new RawApiRefInfo {
                    Namespace = "msg",
                    Name = "Message",
